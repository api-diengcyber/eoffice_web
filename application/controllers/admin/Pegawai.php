...
class Pegawai extends CI_Controller
{
    ...
    public $active = array('active_utilities' => 'active_pegawai');
    ...
    public function setting_wfh($id, $wfh = 0)
    {
        $data_login = $this->Tampilan_model->cek_login();
        $this->Tampilan_model->admin();
        ...
        $this->session->set_flashdata('message', 'Update Wfh Success');
        redirect(site_url('admin/pegawai'));
    }

    public function setting_monitor($id, $monitor = 0)
    {
        $data_login = $this->Tampilan_model->cek_login();
        $this->Tampilan_model->admin();
        $this->Pegawai_model->update($id, [
        ...
        $this->session->set_flashdata('message', 'Update Monitor Success');
        redirect(site_url('admin/pegawai'));
    }
    ...
    public function create()
    {
        ...
        $data = array(
            ...
            'id_jabatan' => '',
            'status' => '',
            'data_kantor' => $this->Kantor_model->get_all(),
        );
    ...
    public function create_action()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array(
                ...
                'status_pernikahan' => $this->input->post("sp", TRUE),
    ...
    public function update($id)
    {
        ...
        if ($row) {
            ...
            $data = array(
                ...
                'sp' => set_value("sp", $row->status_pernikahan),
    ...
    public function update_action()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!empty($_FILES['foto']['tmp_name'])) {
                ...
                if ($this->upload->do_upload('foto')) {
                    $data_img = $this->upload->data();
                    $data['foto'] = $data_img['file_name'];
                    if ($row_pegawai->foto != '') {
                        if (file_exists('assets/photos/' . $row_pegawai->foto)) {
                            unlink('assets/photos/' . $row_pegawai->foto);
                        }
                    }
                } else {
    ...
    public function delete($id)
    {
        ...
        if ($row) {
            ...
            if (!empty($row->foto) && file_exists('assets/photos/' . $row->foto)) {
                unlink('assets/photos/' . $row->foto);
            }
