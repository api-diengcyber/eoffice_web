...
<script>
    ...
    var taskCard = function(params) {
        return `
            <li class="card p-2 mb-1 task-item" data-id="${params.id}" data-url="${params.url}" data-description="${params.description}" data-attachment="${params.attachment}">
    ...
    var taskDetail = function(params) {
        ...
        if (params.attachment) {
            let parseAttachment = JSON.parse(params.attachment);
            for (let item in parseAttachment) {
                let _attachment = parseAttachment[item];
                attachment += `<a href="${_attachment}" target="_blank">${_attachment}</a>`;
            }
        }
        return `
    ...
    $(document).ready(function() {
        ...
        $("body").on("click", ".task-item", function() {
            ...
            let _url = $(this).attr('data-url'),
                _attachment = $(this).attr('data-attachment'),
                url = "",
                attachment = "",
                parseAttachment = JSON.parse(_attachment);
            if (parseAttachment && parseAttachment.length > 0) {
                for (let item in parseAttachment) {
                    let _attachment = parseAttachment[item];
                    attachment += `<a href="${_attachment}" target="_blank">${_attachment}</a>`;
                }
            ...
            if (_url) {
                url += `URL : <a href="${_url}">${_url}</a>`;
            }
            $("#description").html($(this).attr("data-description") + "<br>" + url + "<br>" + attachment);
            getDetail(id);
